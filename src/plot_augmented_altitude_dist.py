"""
Plot altitude distribution with mosaic + random-scale augmentation applied
to the training split, alongside the unaugmented val and test distributions.

For each training frame, N_SAMPLES augmentation factors are drawn:

    apparent_altitude = actual_altitude * 2 / U(1 - scale, 1 + scale)

The factor of 2 models mosaic tiling: each sub-image fills ~half the linear
output dimension, so objects appear at ~0.5x their original pixel size
(apparent altitude ×2). Dividing by U(1-scale, 1+scale) correctly models the
random scale jitter — larger scale → objects appear bigger → lower apparent
altitude. The resulting augmented altitudes are weighted by n_boxes / N_SAMPLES
so total car count is preserved.

Usage
-----
    cd src && python plot_augmented_altitude_dist.py
    cd src && python plot_augmented_altitude_dist.py --scale 0.5
    cd src && python plot_augmented_altitude_dist.py --scale 0.8 --n-samples 200
    cd src && python plot_augmented_altitude_dist.py --out results/aug_alt.png
"""
...
DEFAULT_SCALE = 0.8   # matches AUG['scale'] in train.py
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out", type=Path,
        default=g.RESULTS_DIR / "augmented_altitude_dist.png",
        help="Output path (default: results/augmented_altitude_dist.png)",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Augmentation factor = 4 * U(1-scale, 1+scale)."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        "--no-mosaic", action="store_true", dest="no_mosaic",
        help="Disable the mosaic x4 factor (apply scale jitter only)",
    )
    p.add_argument(
...
def augment_train(
    ...
    """Expand training altitudes by drawing n_samples scale factors per frame.
    ...
    # apparent_altitude = actual * mosaic_factor / U(1-scale, 1+scale)
    # mosaic_factor ≈ 2: each sub-image fills ~half the linear output dimension
    ...
    # Drop non-positive altitudes (can occur when scale >= 1)
    mask = aug_alts > 0
...
def plot_histograms(
    ...
    rows = [
        ("train (augmented)", aug_alts, aug_weights, TRAIN_AUG_COLOR),
...
def main() -> None:
    ...
    mosaic_str = f"2/U(1±{args.scale})" if not args.no_mosaic \
